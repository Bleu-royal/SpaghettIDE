...
\newpage
La boîte à outils XML lxml est une liaison Pythonic pour les bibliothèques C libxml2 et libxslt. Il est unique car il combine la fonctionnalité de la vitesse et de la fonctionnalité XML de ces bibliothèques avec la simplicité d'une API native Python, la plupart du temps compatible mais supérieure à l'API ElementTree bien connue. La dernière version fonctionne avec toutes les versions de CPython de 2.6 à 3.6. Nous avons utilisé la librairie LXML (version 3.7.2) qui fait le lien entre Python et l'XML, qui nous permet d'utiliser des outils de parse XML dans notre IDE avec Python. En effet, l'outil "etree", après son import, nous permet de parser un fichier XML et de le parcourir et modifier par la suite. 
