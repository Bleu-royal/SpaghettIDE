...
Dans la même lignée que le premier semestre, nous avons continué à générer des schémas UML pour que chacun comprenne le fonctionnement de chaque nouveau module, et son utilité. Nous avons pu voir quels objets de Qt nous utilisions ce qui nous a aidé à corriger l'utilisation de certains objets et d'en utiliser d'autres plus appropriés.

		\begin{figure}[h!]
			\begin{center}
				\includegraphics[scale=0.3]{images/imgs_uml/new_relation_Qt.png}
				\caption{Les objets utilisés en fonction des différents modules}
			\end{center}
		\end{figure}
		
Cela nous a aussi permis de comprendre quel module avait besoin de quel autre, et ainsi de ré-arranger nos imports, qui devenaient de plus en plus lourds.
		
		\begin{figure}[h!]
			\begin{center}
				\includegraphics[scale=0.3]{images/imgs_uml/dependances_modules_gui.png}
				\caption{Les dépendances entre nos principaux modules}
			\end{center}
		\end{figure}
		
		\begin{figure}[h!]
			\begin{center}
				\includegraphics[scale=0.3]{images/imgs_uml/dependances_graphique.png}
				\caption{Les dépendances entre les modules principaux}
			\end{center}
		\end{figure}
...
Au fur et à mesure de l'ajout de fonctionnalités dans notre IDE, l'utilisation du XML s'est vite imposée du fait de son efficacité. Nous devions ainsi lire, parcourir et écrire dans un fichier XML afin de subvenir à nos besoins dans l'élaboration des différentes fonctionnalités de notre IDE dans différentes conditions. Nous utilisons plusieurs fichiers xml, un pour chaque projet, un pour tous les projets et un fichier de configuration.
