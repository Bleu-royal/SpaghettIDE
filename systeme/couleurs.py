...
class HTMLHighLighter(QSyntaxHighlighter):
    def __init__(self, parent=None):  # Parent --> QTextEdit
        ...
        self.balises = ('HTML', 'META', 'LINK', 'FORM', 'A', 'INPUT', 'STYLE', 'SCRIPT', 'BODY', 'HEAD', 'NAV', 'TITLE',
                        'BR', 'DIV', 'SPAN', 'H1', 'H2', 'H3', 'H4', 'H5', 'H6', 'IMG', 'P', 'UL', 'LI', 'BUTTON',
                        "LABEL", "ASIDE", "SECTION", "NAV", "HEADER", "FOOTER", "ARTICLE", "EM")
        self.attributs = ('SRC', 'HREF', 'CLASS', 'ID', 'CHARSET', 'REL', 'TITLE', 'TYPE', 'NAME', 'VALUE', "LANG",
                          "INTEGRITY", "CROSSORIGIN", "DATA-TOGGLE", "DATA-TARGET", "ARIA-EXPANDED", "ARIA-CONTROLS",
                          "FOR", "CONTENT")
    ...
    def select_n_color(self, exp_reguliere, color, text, word=False):
        ...
        while pos != -1:  # Tant qu'on est pas au bout de la ligne (text)
            if word:
                ...
                self.setFormat(start, len(word), QColor.fromRgb(color[0], color[1], color[2]))
                # Modificateur du texte sélectionné
            else:
                self.setFormat(pos, rgx.matchedLength(), QColor.fromRgb(color[0], color[1], color[2]))
                # Modificateur du texte sélectionné
    ...
    def highlightBlock(self, text):  # Appelée lorsqu'on change du texte dans le QTextEdit
        ...
        for e in self.balises:
            self.select_n_color("<(/)*%s(\s|>|/>)+(\w|\s|:|;|=|\"|\d|\.|/|\+|-|,|#)*(/)*(>)*" % e,
                                (238, 38, 114), text, e)
            # Balises
